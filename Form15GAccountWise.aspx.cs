...
public partial class Form15GAccountWise : System.Web.UI.Page
{
    Log objLog = new Log();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    ViewState["Pagename"] = "Form15GReport";
                    fillValue();
        ...
        {
            objLog.WriteLog("Form15GAccountWise ->Page_Load(): Unable to load...." + ex.Message);
        ...
        objLog.WriteLog("Form15GAccountWise:Page Loaded Successfully");
    }
    protected void fillValue()
    {
        ...
        bool blnAccWise = false, blnClient = false;
        ...
        try
        {
            ...
            if (dtForm15Det.Rows.Count > 0)
            {
                ...
                if (Session["blnClient"] != null)
                {
                    blnClient = (bool)Session["blnClient"];
                }
                if ((Session["blnAccWise"] != null) && (Session["strAccno"] != null) && (Session["strAbbr"] != null))
                {
                    blnAccWise = (bool)Session["blnAccWise"];
                    strAccno = (string)Session["strAccno"];
                    strAbbr = (string)Session["strAbbr"];
                }
                if (blnClient == true)
                {
                    foreach (DataRow rgrd1 in dtForm15Det.Rows)
                    {
                        ...
                        {
                            if (rgrd1["FORM_TYPE"].ToString().Trim() == "0" || rgrd1["FORM_TYPE"].ToString().Trim() == "F15G")
                            {
                    ...
                    foreach (DataRow rgrd1 in dtForm15Det.Rows)
                    {
                        if (Convert.ToDouble(rgrd1["N_BAL"]) > 0 && Convert.ToDouble(rgrd1["N_PAYABLE"]) > 0)
                        {
                            if (rgrd1["FORM_TYPE"].ToString().Trim() == "0" || rgrd1["FORM_TYPE"].ToString().Trim() == "F15G")
                            {
                                ...
                                if ((rgrd1["N_ABBR"].ToString().Trim() == strAbbr.ToString().Trim()) && (rgrd1["N_ACNO"].ToString().Trim() == strAccno.ToString().Trim()))
                                {
                                    grdmn1 = dtgrdsmmry.NewRow();
                                    grdmn1["Name and address of the person to whom the are given on interest"] = "";
                                    grdmn1["Account No"] = rgrd1["N_ABBR"] + "-" + rgrd1["N_ACNO"] + "(" + rgrd1["N_BRANCH"] + ")";
        ...
        {
            objLog.WriteLog("Inside Form15GAccountWise->fillvalue()" + ex.Message);
        ...
        objLog.WriteLog("Form15GAccountWise->fillvalue() Executed successfully");
    }
    ...
    protected void getBankDet()
    {
        ...
        objLog.WriteLog("Inside Form15GAccountWise->getBankDet()->BankInfo query" + dtBankDet.Rows.Count);
        if (dtBankDet.Rows.Count > 0)
        {
            Session["dtBankDet"] = dtBankDet;
            if ((!DBNull.Value.Equals(dtBankDet.Rows[0]["BAN_NAME"].ToString())))
            {
                objLog.WriteLog("Form15GAccountWise dtBankDet(BAN_NAME)" + dtBankDet.Rows[0]["BAN_NAME"].ToString());
            ...
            {
                objLog.WriteLog("Form15GAccountWise dtBankDet(BAN_ADDRESS1)" + dtBankDet.Rows[0]["BAN_ADDRESS1"].ToString());
            ...
            objLog.WriteLog("Form15GAccountWise->getBankDet()->Bankinfo executed successfully");
        }
        else
        {
    ...
    protected void getClientDet()
    {
        ...
        objLog.WriteLog("Form15GAccountWise->getClientDet()->executed successfully");
    }


    protected void btnBack_Click(object sender, EventArgs e)
    {
        Response.Redirect("Form15HLogAccountWise.aspx");
    }
