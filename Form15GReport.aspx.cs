//**---------------------------FORM15G REPORT PRINTING---------------------------
/*
 * Version    Created By         Date       Modified date  Assigned By
 * 1.1        Harshith        05.05.2015      13.05.2015    Venkatesh 
 * -----------------------------------------------------------------------
 * -----------------------------PSEUDOCODE--------------------------------
 
 * * Purpose of this webform is to print the Form15G report for selected client.
 * 
 * ----------Conditions to Apply Form15H--------------
 * Client is having Age < 60,he must having the PAN No and it is marked as Form15G.
 * 
**-----------TABLES USED----------------
 *BANKINFO
 *CLIENTMASTER 
...
public partial class Form15GReport : System.Web.UI.Page
{
    Log objLog = new Log();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        objLog.WriteLog("Form15GReport:Page Loaded Successfully");
    }
    protected void fillValue()
    {
                ...
                foreach (DataRow rgrd1 in dtForm15Det.Rows)
                {
                    if (Convert.ToDouble(rgrd1["N_BAL"]) > 0 && Convert.ToDouble(rgrd1["N_PAYABLE"]) > 0)
                    {
                        if (rgrd1["FORM_TYPE"].ToString().Trim() == "0" || rgrd1["FORM_TYPE"].ToString().Trim() == "F15G")
                        {
                            ...
                            grdmn1["Date on which sums given on Interest (dd/mm/yyyy)"] = String.Format("{0:dd/MM/yyyy}", rgrd1["N_MDATE"]).ToString();
                            grdmn1["Period for which sums were given on interest"] = String.Format("{0:dd/MM/yyyy}", rgrd1["N_SDATE"]).ToString() + " - " + String.Format("{0:dd/MM/yyyy}", rgrd1["N_MDATE"]).ToString();
        ...
        {
            objLog.WriteLog("Inside Form15GReport->fillvalue()" + ex.Message);
    ...
    protected void getBankDet()
    {
        ...
        objLog.WriteLog("Inside Form15GReport->getBankDet()->BankInfo query" + dtBankDet.Rows.Count);
        if (dtBankDet.Rows.Count > 0)
        {
            Session["dtBankDet"] = dtBankDet;
            if ((!DBNull.Value.Equals(dtBankDet.Rows[0]["BAN_NAME"].ToString())))
            {
                objLog.WriteLog("Form15GReport dtBankDet(BAN_NAME)" + dtBankDet.Rows[0]["BAN_NAME"].ToString());
            ...
            {
                objLog.WriteLog("Form15GReport dtBankDet(BAN_ADDRESS1)" + dtBankDet.Rows[0]["BAN_ADDRESS1"].ToString());
            ...
            objLog.WriteLog("Form15GReport->getBankDet()->Bankinfo executed successfully");
        }
        else
        {
    ...
    protected void getClientDet()
    {
        ...
        objLog.WriteLog("Form15GReport->getClientDet()->executed successfully");
    }
